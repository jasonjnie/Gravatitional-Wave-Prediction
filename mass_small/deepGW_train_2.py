...
def tower_loss(scope, inputs, labels):
    ...
    inputs = deepGW_2.convert_to_tensor_float(inputs)
    labels = deepGW_2.convert_to_tensor_float(labels)

    """
    pred, logits = deepGW_2.inference_mass_predictor(inputs)

    _ = deepGW_2.loss(logits, labels)

    _ = deepGW_2.accuracy(pred, labels)
    """

    pred = deepGW_2.inference_mass_estimator_4conv(inputs)
    _ = deepGW_2.loss_estimator(pred, labels)
    _ = deepGW_2.accuracy_estimator(pred, labels)
...
def train(inputs, labels, num_gpus, num_step):
    ...
    with tf.device('/cpu:0'):
        ...
        # 	inputs, labels = deepGW_2.generate_batch_input(data=inputs, phase='train', snr=snr, size=2*len(inputs), num_epoch, epoch)

        # 	inputs, labels = deepGW_2.generate_batch_input_estimator(inputs, labels, 'train', snr, len(inputs), num_epoch, epoch)
        # 	num_step = inputs.shape[0] // (train_step_size * num_gpus)
        # 	input_epoch, label_epoch = deepGW_2.prepare_data(inputs, labels)
        ...
        for step in range(num_step):
            snr = SNRs[step]
            input_epoch, label_epoch = deepGW_2.generate_batch_input_estimator(inputs, labels, 'train', snr,
                                                                             num_gpus*train_step_size, 0, 0)
            input_batch, label_batch = deepGW_2.get_a_batch(input_epoch, label_epoch, train_step_size, num_gpus, 0)
    ...
    # inputs, labels = deepGW_2.read_dataset(phase='val')
    # test_loss = []
    # test_acc = []
    ...
    # 	testsig, testlabel = deepGW_2.generate_batch_input_estimator(inputs, labels, 'test', snr, 1000, 0, 0)
...
if __name__ == "__main__":
    inputs, labels = deepGW_2.read_dataset(phase='train')    # input shape = (9861, 8192)
    print("")
    '''
    loss, acc = [], []
    for i in range(len(all_num_gpus)):
