...
def get_a_batch(inputs, labels, batch_size, num_gpus, step):
    ...
    label_batch = np.zeros([batch_size, num_gpus, 2])		# [mass1, mass2]
...
def generator_fix_snr_estimator(signal_, label_, size, snr):
    ...
    for i in range(size):		###########################
        r = random.randint(0, 1638)		# add time delay between 0 and 1638
        signal[i, 0:8191-r:1] = signal[i, r:8191:1] * snr
        signal[i] += np.random.normal(0, 1, 8192)	# add noise
        signal[i] = signal[i] / np.std(signal[i])	############????????????????
...
def loss_estimator(logits, labels):   ##########????????
    ...
    tf.add_to_collection('losses', mse)   ##########???????? shape
