...
def make_plot(loss, acc):
    ...
    for i in range(len(all_num_gpus)):
        ...
        sio.savemat('/home/abc99lr/Gravatitional-Wave-Prediction/mass_small/mat/test_loss_' + str(i) + '_' + num_gpu + '_gpu.mat', {'cross_entropy': loss[i]})
        sio.savemat('/home/abc99lr/Gravatitional-Wave-Prediction/mass_small/mat/test_acc_' + str(i) + '_' + num_gpu + '_gpu.mat', {'accuracy': acc[i]})
...
def test(inputs, labels, num_gpus):
    ...
    inputs_tensor = deepGW_2.convert_to_tensor_float(X[:, 0, :])
    labels_tensor = deepGW_2.convert_to_tensor_float(Y_[:, 0, :])

    pred = deepGW_2.inference_mass_estimator_4conv(inputs_tensor)    # same model for inference
    ...
    loss = deepGW_2.loss_estimator(pred, labels_tensor)
    acc = deepGW_2.accuracy_estimator(pred, labels_tensor)
    ...
    for i in range(SNR_num):
        snr = test_snr[i]
        # print("SNR = ", snr)
        input_epoch, label_epoch = deepGW_2.generate_batch_input_estimator(inputs, labels, 'test', snr,
                                                                         num_gpus*test_step_size, 0, 0)
        input_batch, label_batch = deepGW_2.get_a_batch(input_epoch, label_epoch, test_step_size, num_gpus, 0)
...
if __name__ == "__main__":
    inputs, labels = deepGW_2.read_dataset(phase='test')
    loss, acc = [], []
    for i in range(len(all_num_gpus)):
